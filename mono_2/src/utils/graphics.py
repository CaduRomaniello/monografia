...
# LAHC-mono
def generate_graphic_lahc_mono(original_solution, best_solution, graphics):
    max_idleness, max_standing, max_deallocated = get_normalization_values(original_solution)

    original_idleness = [individual['objectives'].idleness / max_idleness for individual in graphics['original']]
    original_deallocated = [individual['objectives'].deallocated / max_deallocated for individual in graphics['original']]
    original_standing = [individual['objectives'].standing / max_standing for individual in graphics['original']]

    best_idleness = [individual['objectives'].idleness / max_idleness for individual in graphics['best']]
    best_deallocated = [individual['objectives'].deallocated / max_deallocated for individual in graphics['best']]
    best_standing = [individual['objectives'].standing / max_standing for individual in graphics['best']]

    fig = go.Figure(data=[go.Scatter3d(x=original_idleness, y=original_deallocated, z=original_standing,mode='markers'), go.Scatter3d(x=best_idleness, y=best_deallocated, z=best_standing,mode='markers', marker=dict(color='green'))])
    # fig.update_layout(xaxis_title="Idleness", yaxis_title="Deallocated", zaxis_title="Standing")
    fig.update_layout(
        scene = dict(
            xaxis = dict(nticks=10, range=[0, 1],),
